...
class AddBookingteam extends StatefulWidget {
  final List list;
  final int index;
  final team_id;
  const AddBookingteam({this.list, this.index, this.team_id});

  @override
  _AddBookingteamState createState() => _AddBookingteamState();
}

class _AddBookingteamState extends State<AddBookingteam> {
  ...
  Future uploadRoom() async {
    ...
    if (json.decode(request.body) == "Error") {
      ...
      sendmail().sendmailer(
          name: data3[0]['firstname'],
          email: data3[0]['email'],
          subject: "Booking meeting room",
          status: 'team',
           reserve_id : data[0]['reserve_id'],
          body:
              "${data3[0]['firstname']} ${data3[0]['lastname']} have booking the meeting room ${widget.list[widget.index]['name']} in ${DateFormat('dd MMMM yyy').format(DateTime(bookdate.year, bookdate.month, bookdate.day))} at ${DateFormat('HH:mm').format(bookingin).toString()} - ${DateFormat('HH:mm').format(bookingout).toString()}");

      String teamname = '';
      final response2 = await http.get(Uri.parse(
          "http://${ipconn}/letsmeet/teams/getteamname.php?team_id=${team_id}"));
      var data2 = json.decode(response2.body);
      print(data2);
      teamname = data2[0]['team_name'];
      // print(''+teamname);
